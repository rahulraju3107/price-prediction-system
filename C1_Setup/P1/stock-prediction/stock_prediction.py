...
def load_data(ticker, n_steps=50, scale=True, shuffle=True, lookup_step=1, split_by_date=True,
    ...
    if isinstance(ticker, str):
        ...
        df = yf.download(ticker, progress=False, auto_adjust=False, start="2000-01-01")
        # rename columns to match expected format
        if isinstance(df.columns, pd.MultiIndex):
            df.columns = [col[0].lower().replace(' ', '') for col in df.columns]
        else:
            df.columns = [str(col).lower().replace(' ', '') for col in df.columns]
        df = df.rename(columns={'adjclose': 'adjclose', 'close': 'close'})
    ...
    # check if sequence_data is empty
    if len(sequence_data) == 0:
        raise ValueError(f"No sequences generated. DataFrame has {len(df)} rows, which is less than n_steps ({n_steps}) + lookup_step ({lookup_step}). Try reducing these parameters or increasing the data range.")
    ...
    if split_by_date:
        ...
        if train_samples == 0:
            raise ValueError("Not enough samples for training. Increase dataset size or reduce n_steps/lookup_step.")
